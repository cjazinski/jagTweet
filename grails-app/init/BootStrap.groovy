...
class BootStrap {
    ...
    private createSampleData() {
        ...
        def chris = new User(
        		username:"cjazinski",
        		password:"password",
        		profile: new Profile(fullName:"Christopher Jazinski", email: "chris@jazinski.com"),
        		dateCreated: now
        ...
        def phil = new User(
                ...
                dateCreated: now)
        def dillon = new User(username: "dillon",
                password: "crikey",
                profile: new Profile(fullName: "Dillon Jessop", email: "dillon@nowhere.net"),
                dateCreated: now - 2).save(failOnError: true)

        chris.addToFollowing(chuck)
        chris.addToPosts(content: "Been working on JagTweet")
        chris.addToPosts(content: "Working on getting beer! Mmmmm Beer")
        chris.addToPosts(content: "I <3 Doughnuts")
        chris.save()
        ...
        postsAsList[0].dateCreated = now.updated(year: 2004, month: 2)
        ...
        postsAsList[1].dateCreated = now.updated(year: 2007, month: 3, date: 13)

        postsAsList[2].addToTags(user: phil, name: "groovy")
        postsAsList[2].addToTags(user: phil, name: "bbq")
        postsAsList[2].dateCreated = now.updated(year: 2009, month: 10)

        postsAsList[3].addToTags(user: phil, name: "groovy")
        postsAsList[3].dateCreated = now.updated(year: 2011, month: 5, date: 1)

        postsAsList[4].dateCreated = now.updated(year: 2011, month: 12, date: 4)
        postsAsList[5].dateCreated = now.updated(year: 2012, date: 10)
        phil.save(failOnError: true)

        postsAsList = sara.posts as List
        postsAsList[0].dateCreated = now.updated(year: 2007, month: 5)
        postsAsList[1].dateCreated = now.updated(year: 2008, month: 4, date: 13)
        postsAsList[2].dateCreated = now.updated(year: 2008, month: 4, date: 24)
        postsAsList[3].dateCreated = now.updated(year: 2011, month: 11, date: 8)
        postsAsList[4].dateCreated = now.updated(year: 2011, month: 12, date: 4)
        postsAsList[5].dateCreated = now.updated(year: 2012, month: 8, date: 1)
    ...
    private createAdminUserIfRequired() {
        ...
        if (!User.findByUsername("admin")) {
            ...
            new User(username: "admin", password: "secret", profile: profile).save(failOnError: true)
        }
        else {
